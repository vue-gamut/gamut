...
import { CollectionStateBase, Key, Node } from "@vue-types/shared";
import { PartialNode } from "./types";
import { VNode, Fragment, isVNode } from "vue";

interface CollectionBuilderState {
  renderer?: (value: any) => VNode | null;
}

interface CollectVueNode<T> {
  getCollectionNode(
    props: any,
    context: any
  ): Generator<PartialNode<T>, void, Node<T>[]>;
}
...
export class CollectionBuilder<T extends object> {
  ...
  build(
    props: Partial<CollectionStateBase<T>>,
    context?: unknown
  ): Iterable<Node<T>> {
  ...
  private *iterateCollection(
    ...
    if (isVNode(children) && children.type === Fragment) {
      const childrenArray = Array.isArray(children.children)
        ? children.children
        : [children.children];
      yield* this.iterateCollection({
        children: childrenArray.filter(isVNode),
        items,
      });
      ...
      if (Array.isArray(children)) {
        nodeList = children.filter(isVNode);
  ...
  private *getFullNode(
    ...
    if (isVNode(partialNode.element) && partialNode.element.type === Fragment) {
      const children = Array.isArray(partialNode.element.children)
        ? partialNode.element.children.filter(isVNode)
        : isVNode(partialNode.element.children)
        ? [partialNode.element.children]
        : [];
    ...
    if (isVNode(element)) {
      let type = element.type as unknown as CollectVueNode<T>;
      if (
          ...
          if (partialNode.type && node.type !== partialNode.type) {
            throw new Error(
              `Unsupported type <${capitalize(node.type)}> in <${capitalize(
                parentNode?.type ?? "unknown parent type"
...
type Wrapper = (element: VNode) => VNode;
function compose(
  outer: Wrapper | void,
  inner: Wrapper | void
): Wrapper | undefined {
